...
def build_email_body(temperature):
    """Builds the friendly plain-text email body."""
    body = (
        f"Hello Praveen,\n\n"
        f"The current weather report for the location at latitude {LATITUDE} "
        f"and longitude {LONGITUDE} shows a temperature of {temperature}°C.\n"
        f"The latest weather data was retrieved successfully from Open-Meteo.\n"
        f"This report provides the current available temperature information "
        f"for your selected location.\n\n"
        f"Have a great day!"
    )
    return body
...
def send_email(temperature):
    ...
    subject = "Weather Update for Praveen"
